...
def clean_boxoffice(movie):
    ...
    """
    try:
        movie['BoxOffice'] = int(movie['BoxOffice'].split('$')[1].replace(',', ''))
    except:
        movie['BoxOffice'] = 0
    return movie
#START HERE
def clean_imdb_data(movie):
    ...
    Returns:
        dict: dictionary containing "key: value" pairs representing data for one movie
    """
    movie['imdbVotes'] = int(movie['imdbVotes'].replace(',', '')) #float
    movie['imdbRating'] = float(movie['imdbRating']) #float
    return movie
...
def count_sub_genres(movies):
    ...
    """
    sub_genres = {'Sci-Fi': 0, 'Fantasy': 0, 'Thriller': 0, 'Mystery': 0}
    for movie in movies:
        if 'Sci-Fi' in movie['Genre']:
            sub_genres['Sci-Fi'] += 1
        elif 'Fantasy' in movie['Genre']:
            sub_genres['Sci-Fi'] += 1
        #else 'Mystery' in movie['Genre']:
...
def main():
    ...
    horror_movies = read_csv_to_dicts("./assignments/le_07/horror_movies.csv")
    ...
    # Problem 04
    print("\nProblem 04:")
    #subgenre_counts = count_sub_genres()

    # Problem 05
    print("\nProblem 05:")
    horror_movies_new = None
